...
# Login
r = requests.post(f"{BASE}/auth/login", json={"email":"rudraadmin@gmail.com","password":"admin123"})
...
df = pd.DataFrame({
    ...
    "profit":   np.random.uniform(-50,200,100),
...
CHARTS = [
    ...
    "Bubble","Violin","Funnel","Sunburst","Marimekko","Table with Charts","3D Area",
    # 2026 Trending
    "Sankey","Beeswarm","Bullet","Small Multiples","Rose","Time Series","Infographic",
    "Network Graph","Gantt",
    # Advanced/AI-Era
    "Parallel Coordinates","Chord","Hexbin","Ridgeline","Streamgraph",
    "Dendrogram","Voronoi","Uncertainty","PCA","SHAP","Choropleth","Event Stream",
    # Statistical
    "Q-Q Plot","ECDF","KDE","Rug Plot","Dot Plot","Lollipop","Slope Chart","Dumbbell","Diverging Bar",
    # Business
    ...
    "PDF Plot","Kaplan-Meier","Periodogram","Adjacency Matrix","Pareto",
    "Manhattan","Volcano",
    # Finance & Ops
...
TYPE_MAP = {
    ...
    "Table with Charts":"table_sparkline","3D Area":"3d_area","Marimekko":"marimekko",
...
passed, failed = [], []
for name in CHARTS:
    ct = TYPE_MAP.get(name, name.lower())
    payload = {"chart_type": ct, "x_col": "category", "y_col": "sales", "title": name}
    try:
        r = requests.post(f"{BASE}/viz/custom", json=payload, headers=headers, timeout=15)
        if r.status_code == 200:
            passed.append(name)
        else:
            err = r.json().get("detail","?")[:80] if r.headers.get("content-type","").startswith("application/json") else r.text[:80]
